...
def JourneyResultsEx( \
    naptanFrom="Moor Park Underground Station", \
    # naptanFrom="Kings Cross Underground Station",
    naptanTo="Watford Underground Station", \
    fromName="Moor Park", toName="Amersham", \
    yyyyMMdd="20170610", departTime_hhmm="0830" \
    ):
  import urllib
  q = urllib.quote
  ...
  return url.format(naptanFrom=q(naptanFrom), to=q(naptanTo), \
                    yyyyMMdd=q(yyyyMMdd), time=q(departTime_hhmm), \
                    fromName=q(fromName), toName=q(toName))
